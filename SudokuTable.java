...
public class SudokuTable {
    ...
    private me.tacchino.sudoku.SudokuDigit[][] table = new me.tacchino.sudoku.SudokuDigit[9][9];

    //constructor: create sudokuDigits in all 81 blocks
    public SudokuTable(){
        for (int y = 0; y < 9; y++){
            for (int x = 0; x < 9; x++){
                table[x][y] = new me.tacchino.sudoku.SudokuDigit(x,y);
            }
        }
    }

    //get a digit in the table
    public me.tacchino.sudoku.SudokuDigit getDigit(int x, int y){
        if (x >= 0 && x <= 8 && y >= 0 && y <= 8)
            return table[x][y];
        else
    ...
    boolean checkDigit(me.tacchino.sudoku.SudokuDigit digit) {
        if (!digit.isSolved())
    ...
    boolean solve(int position) {
        ...
        for (int possibleValue : posssibleValues) {
            me.tacchino.sudoku.SudokuDigit digit = new me.tacchino.sudoku.SudokuDigit(table[x][y]); //make a shallow copy of the digit at x,y
    ...
    public int getNumOfDigitsSolved(){
        ...
        for (me.tacchino.sudoku.SudokuDigit[] column : table){
            for (me.tacchino.sudoku.SudokuDigit sudokuDigit : column){
